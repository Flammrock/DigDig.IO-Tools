...
import { Hash, HashAlgorithm } from './definitons'

const md5cycle = function (x: Array<number>, k: Array<number>) {
  let a = x[0],
    b = x[1],
    c = x[2],
    d = x[3]
...
const cmn = function (q: number, a: number, b: number, x: number, s: number, t: number) {
...
const tb = function (x: Array<number>) {
  const a = new Array(x.length * 4)
...
if (MD5.hash('hello').toString() != '5d41402abc4b2a76b9719d911017c592') {
  add32 = function (x, y) {
